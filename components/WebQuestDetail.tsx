...
const WebQuestDetail: React.FC<WebQuestDetailProps> = ({ quest, onClose }) => {
  ...
  const saveResult = () => {
    if (!studentName) {
      alert("⚠️ ¡ALTO! Antes de terminar, debes seleccionar tu nombre de la lista.");
      return;
    }
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/98 backdrop-blur-2xl animate-in fade-in duration-300">
      <div className="max-w-5xl mx-auto my-8 bg-white rounded-[2rem] overflow-hidden text-slate-800 shadow-2xl relative border-8 border-yellow-400">
        ...
        <div className="bg-yellow-400 p-6 flex items-center gap-6 border-b-4 border-slate-800">
          ...
          <h1 className="pixel-font text-2xl md:text-3xl text-slate-800">COLEGIO ALEMÁN HUB</h1>
          ...
          <div className="flex flex-wrap bg-white border-b-2 border-slate-100 p-2 gap-2">
            {[
              ...
              >
                ...
                <div className="font-black text-xs uppercase tracking-widest">{tab.label}</div>
              </button>
            ))}
          </div>
        )}
                ...
                <div className="p-10 bg-emerald-50 rounded-[3rem] border-8 border-emerald-200 text-center">
                  ...
                  <div className="flex flex-wrap justify-center gap-8 mb-10">
                      <div className="bg-white p-8 rounded-[2rem] border-4 border-emerald-500 shadow-xl min-w-[200px]">
                        <p className="text-xs font-black text-slate-400 uppercase mb-2">Puntaje Quiz</p>
                      ...
                      <div className="bg-white p-8 rounded-[2rem] border-4 border-indigo-500 shadow-xl min-w-[200px]">
                        <p className="text-xs font-black text-slate-400 uppercase mb-2">Tu Reflexión</p>
                  ...
                  <div className="flex flex-col md:flex-row gap-4 justify-center">
                    <button onClick={() => setShowReview(true)} className="px-12 py-5 bg-indigo-600 text-white rounded-2xl font-black text-xl hover:bg-indigo-700 transition-all shadow-2xl flex items-center justify-center gap-3">
                      <span>👁️</span> REVISAR MIS RESPUESTAS
                    </button>
                            ...
                            {q.options.map((opt, oIdx) => {
                              ...
                              return (
                                <div key={oIdx} className={`p-4 rounded-xl font-bold border-2 text-sm ${style}`}>
                                  {opt}
                                  {oIdx === quizScores[qIdx] && <span className="ml-2 opacity-70 text-[10px] uppercase font-black">(Tu elección)</span>}
                                  {oIdx === q.correctAnswer && <span className="ml-2 opacity-70 text-[10px] uppercase font-black">(Correcta)</span>}
                                </div>
                              );
                            })}
                          </div>
            ...
            <>
              {activeTab === 'intro' && (
                <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-8">
                  ...
                  <div className="rounded-[3rem] overflow-hidden border-8 border-slate-100 shadow-2xl">
                     <img src={quest.imageUrl} alt={quest.title} className="w-full h-auto object-cover max-h-[450px]" />
                  </div>
                </div>
              )}
              ...
              {activeTab === 'tarea' && (
                <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-8">
                  ...
                  <div className="mt-12 bg-slate-50 p-10 rounded-[3rem] border-4 border-dashed border-slate-200">
                    <h4 className="text-2xl font-black text-slate-800 mb-6 uppercase flex items-center gap-3">
                       <span className="text-4xl">🛠️</span> Materiales y Recursos
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {quest.resources.map((res, i) => (
                        <div key={i} className="flex items-center gap-5 bg-white p-6 rounded-2xl border-2 border-slate-100 shadow-sm hover:border-emerald-300 transition-colors">
              ...
              {activeTab === 'proceso' && (
                <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-8">
                  <h3 className="text-5xl font-black text-emerald-500 mb-10 uppercase italic">El Camino al Éxito</h3>
              ...
              {activeTab === 'evaluacion' && (
                <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-12">
                  <h3 className="text-5xl font-black text-emerald-500 mb-6 uppercase italic">Mini-Quiz de Poder</h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {q.options.map((opt, oIdx) => (
                            <button 
                              ...
                              className={`p-6 rounded-2xl text-left font-bold border-4 text-lg transition-all ${quizScores[qIdx] === oIdx ? 'bg-emerald-600 text-white border-emerald-700 shadow-xl scale-[1.02]' : 'bg-white text-slate-600 border-slate-100 hover:border-emerald-200'}`}
                            >
                              {opt}
                            </button>
                          ))}
